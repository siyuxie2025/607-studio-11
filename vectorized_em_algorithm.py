...
class GaussianMixtureModelVectorized:
    ...
    def _e_step(self, X):
        ...
        for k in range(self.n_components):
            try:
                ...
                log_pdf_values = np.log(pdf_values + 1e-10)
                log_pi_k = np.log(self.pi_[k] + 1e-10)
                log_responsibilities_k = log_pi_k + log_pdf_values
        ...
        responsibilities = responsibilities / np.where(row_sums_responsibilities > 0, 
                                                       row_sums_responsibilities, 
                                                       self.n_components)  
    ...
    def _m_step(self, X, responsibilities):
        ...
        pi_new = Nk / n_samples  # shape (n_components,)
        mu_new = np.dot(responsibilities.T, X) / Nk[:, np.newaxis]
        cov_new = []
        for k in range(self.n_components):
            diff = X - mu_new[k]
    ...
    def _compute_log_likelihood(self, X):
        ...
        pdf_values = [multivariate_normal.pdf(X, mean=self.mu_[k], cov=self.cov_[k]) \
                                             for k in range(self.n_components)]
        # use log-sum-exp trick for numerical stability
        log_pdf_values = np.log(np.array(pdf_values) + 1e-10)  
        log_weighted_pdfs = log_pdf_values + np.log(self.pi_[:, np.newaxis])
        weighted_pdfs = np.exp(log_weighted_pdfs)
        point_likelihoods = np.sum(weighted_pdfs, axis=0)
        log_likelihood = np.sum(np.log(point_likelihoods + 1e-10))
